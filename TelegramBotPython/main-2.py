...
bot_token = "7603107118:AAGIQgyYi2hHlZnTyCu0lSKSvs4FShRr36k"
...
item5 = types.KeyboardButton("Документация по CSS (с developer.mozilla.org)")
...
def MWDCSSReferenceModulesParser(message):
    ...
    if main_sections.count > 0:
        markup1 = types.ReplyKeyboardMarkup(resize_keyboard=True)
        markup1.row(main_sections)
        bot.send_message("Выберите раздел статьи:", reply_markup=markup1)
...
def message_reply(message):
    if message.chat.id in accessUsers:
        ...
        # elif message.text == 'Документация по CSS (с developer.mozilla.org)':            
